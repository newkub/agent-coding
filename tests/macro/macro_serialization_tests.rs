...
use agent_tui::modules::macros::domain::models::{Macro, MacroContext, MacroStep, MacroId};

#[test]
fn test_macro_serialization() {
    let mut macro_def = Macro::create(MacroId::from_string(uuid::Uuid::new_v4().to_string()), "Test".to_string(), "desc".to_string(), chrono::Utc::now());
