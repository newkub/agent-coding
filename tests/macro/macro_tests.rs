...
#[test]
fn test_macro_clone() {
    let mut m1 = Macro::create(
        MacroId::from_string(uuid::Uuid::new_v4().to_string()),
        "Test".to_string(),
        "desc".to_string(),
        chrono::Utc::now(),
    );
    m1.add_step(MacroStep::Input {
        text: "test".to_string(),
    });
