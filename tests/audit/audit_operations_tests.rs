...
fn test_generate_audit_summary_with_entries() {
    let entries = vec![
        create_test_audit_entry(
            AuditAction::FileRead { path: "a.txt".to_string() },
        ...
        create_test_audit_entry(
            AuditAction::FileRead { path: "b.txt".to_string() },
            Actor { type_: ActorType::User, id: "1".to_string(), name: "User".to_string() },
            Resource { type_: "file".to_string(), id: "2".to_string(), path: None },
        ).with_result(AuditResult::Failure { error: "error".to_string() }),
    ];
...
fn test_audit_rule_matches_action() {
    ...
    let action = AuditAction::CommandExecute { command: "ls".to_string() };
    
    let actor = Actor { type_: ActorType::User, id: "any".to_string(), name: "Test".to_string() };
...
fn test_audit_rule_matches_actor() {
    ...
    let action = AuditAction::FileRead { path: "test".to_string() };
    
    let matching_actor = Actor { type_: ActorType::User, id: "user-1".to_string(), name: "User".to_string() };
    assert!(rule.matches(&action, &matching_actor));
    
    let non_matching_actor = Actor { type_: ActorType::User, id: "user-2".to_string(), name: "User".to_string() };
...
fn test_is_action_allowed_no_rules() {
    let action = AuditAction::FileRead { path: "test".to_string() };
    let actor = Actor { type_: ActorType::User, id: "1".to_string(), name: "Test".to_string() };
