...
pub(crate) async fn import_session<R>(repo: &R, json: &str) -> AppResult<ImportResult>
where
    R: ShareRepository,
{
    let exported: ExportedSession = serde_json::from_str(json).map_err(|e| {
        crate::shared::kernel::result::AppError::State(format!("Invalid JSON: {}", e))
    })?;

    // Check version compatibility
    if exported.version != "1.0" {
        return Err(crate::shared::kernel::result::AppError::State(format!(
