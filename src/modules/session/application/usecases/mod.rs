use crate::modules::session::ports::{SessionRepository, SessionEventPublisher};
...
use crate::shared::kernel::result::{AppResult, AppError};
    ...
    E: SessionEventPublisher,
{
    ...
    publisher.publish(SessionEvent::Created { session_id: session.id.0.clone() }).await?;
    Ok(session)
}
...
pub(crate) async fn list_sessions<R>(
    repo: &R,
...
pub(crate) async fn delete_session<R, E>(
    repo: &R,
    publisher: &E,
    id: &SessionId,
) -> AppResult<()>
    ...
    E: SessionEventPublisher,
{
    ...
    publisher.publish(SessionEvent::Closed { session_id: id.0.clone() }).await?;
    Ok(())
}
    ...
    E: SessionEventPublisher,
{
    ...
    repo.save(&session).await?;
    publisher.publish(SessionEvent::MessageAdded { 
