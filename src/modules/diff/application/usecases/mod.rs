...
use crate::modules::diff::ports::{DiffParser, DiffEventPublisher};
use crate::shared::kernel::result::AppResult;

/// Use case: Parse and review diff
pub(crate) async fn review_diff<P>(
    parser: &P,
    diff_text: &str,
) -> AppResult<DiffReview>
where
    P: DiffParser,
{
...
pub(crate) async fn approve_hunk<P>(
    ...
    P: DiffEventPublisher,
{
    ...
    if let (Some(f), Some(h)) = (file, hunk_id) {
        publisher.publish(DiffEvent::HunkApproved {
            file_path: f.path,
            hunk_id: h,
        }).await?;
    }
...
pub(crate) async fn reject_hunk<P>(
    review: &mut DiffReview,
    publisher: &P,
) -> AppResult<()>
where
    P: DiffEventPublisher,
{
    ...
    if let (Some(f), Some(h)) = (file, hunk_id) {
        publisher.publish(DiffEvent::HunkRejected {
            file_path: f.path,
            hunk_id: h,
        }).await?;
    }
...
pub(crate) async fn apply_approved_changes(
    review: &DiffReview,
    ...
    for file in &review.files {
        let approved_count = file.hunks.iter()
