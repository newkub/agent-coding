...
use crate::modules::snippet::domain::operations::{validate_snippet_name, validate_snippet_code};
...
pub(crate) fn create_snippet(
    ...
    validate_snippet_name(&name).map_err(|e| 
        crate::shared::kernel::result::AppError::State(e.to_string())
    )?;
    validate_snippet_code(&code).map_err(|e|
        crate::shared::kernel::result::AppError::State(e.to_string())
    )?;
    
    // Create snippet with generated ID and timestamps
    let id = crate::modules::snippet::domain::models::SnippetId::from_string(uuid::Uuid::new_v4().to_string());
    ...
    snippet.variables
        ...
        .map(|v| {
            let prompt = format!(
                "Enter value for '{}':{}",
                v.name,
                v.description.as_deref().map(|d| format!(" ({})", d)).unwrap_or_default()
            );
