...
pub fn calculate_risk_score(command: &str) -> u32 {
    ...
    if cmd_lower.contains("chmod 7") {
        score += 30;
    }
    if cmd_lower.contains("curl") || cmd_lower.contains("wget") {
        score += 20;
    }
    if cmd_lower.contains("| sh") || cmd_lower.contains("> /") {
        score += 50;
    }
    if cmd_lower.contains("git push") || cmd_lower.contains("git force") {
        score += 25;
    }
    if cmd_lower.contains("npm install") || cmd_lower.contains("pip install") {
        score += 15;
    }
    if cmd_lower.contains("docker run") {
        score += 20;
    }
    if cmd_lower.contains("exec") {
        score += 25;
    }
    ...
    if cmd_lower.contains("ssh") || cmd_lower.contains("scp") {
        score += 35;
    }
